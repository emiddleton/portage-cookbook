  ...
  module Portage
    module Emerge
      ...
      def conditional_emerge(new_resource, action)
        ...
        # only merge with newuse if not new version
        options = new_resource.options.to_s
        if not emerge?(action, package_data, new_resource.version)
          options << ' --newuse '
        end
        emerge(package_data[:package_atom], options)
      end
  ...
  class Provider
    class Package
      class Portage < Chef::Provider::Package
        ...
        # TODO: fix change reporting as this will allways report a new install
