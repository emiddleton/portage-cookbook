...
define :make_conf, :appends => [], :overrides => [], :sources => [], :force_regen => false do
  ...
  if params[:force_regen]
    ruby_block 'update make.conf' do
      block do
...
define :layman_cfg, :overlay => 'http://www.gentoo.org/proj/en/overlays/repositories.xml' do
  include_recipe 'portage::layman'

  execute 'layman-fetch' do
    command '/usr/bin/layman --fetch'
    action :nothing
  end

  t = nil
  begin
    t = resources(:template => "/etc/layman/layman.cfg")
  rescue Chef::Exceptions::ResourceNotFound
    t = template "/etc/layman/layman.cfg" do
      source "layman.cfg.erb"
      cookbook "portage"
      owner "root"
      group "root"
      mode "0644"
      backup 0
      variables :overlays => node[:portage][:layman][:urls]
      notifies :run, resources(:execute => "layman-fetch")
    end
  end

  t.variables[:overlays].push(params[:overlay]).sort!.uniq!
  node[:portage][:layman][:urls].push(params[:overlay]).sort!.uniq!
end
